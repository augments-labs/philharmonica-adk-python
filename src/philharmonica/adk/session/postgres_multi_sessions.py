...
Install with ``pip install 'philharmonica-adk[session-postgres]'``.
"""
...
try:
    ...
    raise ImportError(
        "PostgresMultiSessions requires psycopg: pip install 'philharmonica-adk[session-postgres]'",
        name="psycopg",
    ) from exc

from philharmonica.adk.session.multi_sessions import MultiSessions
from philharmonica.adk.session.postgres_session import (
    PostgresSession,
    ensure_schema,
    get_app_state,
...
from philharmonica.adk.session.state import APP_PREFIX, State
