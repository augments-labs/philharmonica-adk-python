"""``philharmonica sessions`` — inspect and prune SQLite session stores.

Operates on the same stores ``philharmonica run --session-db`` and
``philharmonica chat --session-db`` write. The store is app-scoped, so every
