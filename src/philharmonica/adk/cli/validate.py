"""``philharmonica validate`` — check a config file without spending a token.
...
schemas, providers) actually import — the same path ``philharmonica run`` takes,
minus execution.
"""
...
from philharmonica.adk.cli.loading import ConfigKind, detect_config_kind
...
def validate(config: Path, kind: ConfigKind | None, resolve_refs: bool) -> None:
    ...
    from philharmonica.adk.config.loader import read_config_document
...
def _schema_check(path: Path, data: dict[str, Any], kind: ConfigKind) -> str:
    ...
    if kind == "topology":
        from philharmonica.adk.types.config.topology_config import TopologyConfig
...
def _assemble(path: Path, kind: ConfigKind) -> str:
    ...
    if kind == "topology":
        from philharmonica.adk.config.topology import load_topology

        topology = load_topology(path)
        names = ", ".join(sorted(topology.agents))
        return f"topology ({len(topology.agents)} agents: {names}) resolved"

    from philharmonica.adk.config.loader import load_agent
