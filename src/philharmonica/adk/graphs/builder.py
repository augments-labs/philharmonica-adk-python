...
1. **Readability first.** A graph definition should read top-to-bottom
   ...
   - Strands: ``GraphBuilder().add_node(agent, "x").add_edge("x", "y")``
     ...
     versions.
   - Philharmonica (this file): ``Graph.new("x").node("a", agent).node("b", agent).pipe("a", "b").entry("a").terminal("b").compile()``.
...
from philharmonica.adk.graphs.merge import DEFAULT_MERGE, MergeFn
from philharmonica.adk.graphs.node import EdgeCondition, GraphEdge, GraphNode, NodeErrorHandlerFn
