...
A ``TaskPipeline`` is a frozen, declarative ordering of
``Task`` instances. The Runner executes them in sequence; each
task's ``Task.description`` IS its user prompt, with no runtime
override. The pipeline aggregates per-task usage into a single
``TaskPipelineResult.context`` and supports conditional skip via
``Task.skip_if``.
...
explicitly: run task A via ``Runner.arun_task``, then construct
...
- You want N tasks to run in order with conditional ``Task.skip_if``
  skips.
- You want cumulative LLM usage across the whole sequence in one
  ``TaskPipelineResult.context``.
...
from philharmonica.adk.tasks.task_output import _output_preview
...
class TaskPipeline[TContext]:
    """Sequential composition of ``Task`` instances.

    Each task's ``Task.description`` is its user prompt verbatim;
    the pipeline does NOT transform prompts at runtime. To forward an
    upstream output into a downstream task's prompt, call
    ``Runner.arun_task`` for the upstream task first, then
    construct the downstream ``Task`` with a description that
    includes the prior output, then call ``Runner.arun_task``
    ...
    Example:
        ::
            ...
            from philharmonica.adk import Agent, Task, TaskPipeline, Runner
    ...
    def __repr__(self) -> str:
        ...
        ``Task.depends_on`` — the signal that execution runs in
    ...
    def __post_init__(self) -> None:
        """Validate ``TaskPipeline`` construction.

        Triggers full DAG validation (duplicate / unknown / missing
        IDs, cycles) when any task declares ``Task.depends_on``.
        ...
        Raises:
            ...
            TaskPipelineDefinitionError: When the pipeline declares an
                ...
                ``philharmonica.adk.exceptions.exceptions.UserError``.
        """
        if len(self.tasks) == 0:
        ...
        if any(t.depends_on is not None and len(t.depends_on) > 0 for t in self.tasks):
            from philharmonica.adk.tasks.topology import topological_levels
...
class TaskPipelineResult[TContext]:
    ...
    def __repr__(self) -> str:
        ...
        ``task_outputs`` — the same slots a human would scan to
