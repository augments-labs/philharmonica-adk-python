"""``philharmonica deploy`` — generate deployment artifacts and ship the agent.
...
from philharmonica.adk.deploy.commands import DeployCommandFailed, DeployToolMissing, SubprocessRunner
from philharmonica.adk.deploy.context import DeployContext
from philharmonica.adk.deploy.helm_chart import split_image_reference
from philharmonica.adk.deploy.targets import TARGETS
...
def context_options[F: Callable[..., object]](f: F) -> F:
    ...
    f = click.option(
        "--image", default="philharmonica-agent:latest", show_default=True, help="Container image name[:tag]."
    )(f)
    f = click.option(
    ...
    f = click.option(
        "--extras", default="serve,a2a", show_default=True, help="philharmonica-adk extras installed in the image."
    )(f)
    f = click.option(
        "--env-key",
        "env_keys",
...
def _context(
    ...
    Args:
        ...
        port: Container port.
        extras: ``philharmonica-adk`` extras to install.
...
def init(
    ...
    click.echo("")
    click.echo("Next: make requirements.txt install philharmonica-adk, then build the image:")
    click.echo(f"  philharmonica deploy build --agent {agent_ref} --image {ctx.image}")
