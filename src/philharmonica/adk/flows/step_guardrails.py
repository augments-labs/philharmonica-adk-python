...
Step-level analogue of ``philharmonica.adk.tools.tool_guardrails.ToolGuardrails``
but operating on ``FlowStepContext`` (pre) and the developer's
...
- ``reject_content(message)`` — short-circuit with a routed explanation
  (lands on a ``FlowStepRejectedEvent`` and routes through
...
if TYPE_CHECKING:
    from philharmonica.adk.flows.step_context import FlowStepContext


@dataclass(frozen=True, kw_only=True)
class FlowStepGuardrailVerdict:
    """Typed verdict returned by a ``FlowStepGuardrailFn``.

    Mirrors the verdict shape used by
    ``philharmonica.adk.tools.tool_guardrails.ToolGuardrailFunctionOutput``:
    callers construct one of the three factory variants
    (``allow``, ``reject_content``,
    ``raise_exception``) rather than wiring booleans.
    ...
    Attributes:
        ...
        exception: When set, the verdict raises this exception
            ...
            Used by the ``raise_exception`` factory.
    """
...
Receives a ``FlowStepContext`` for the step about to run
(pre-guardrail) or that just ran (post-guardrail) and returns a
``FlowStepGuardrailVerdict``. Sync or async — the executor
...
class FlowStepGuardrails:
    ...
    ``philharmonica.adk.tools.tool_guardrails.ToolGuardrails``. Pre runs after
