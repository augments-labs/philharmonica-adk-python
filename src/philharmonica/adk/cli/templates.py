    ...
    """\
{
  ...
  "name": "$name",
  "description": "Starter agent scaffolded by the Philharmonica CLI.",
...
A starter Philharmonica ADK agent project.

```bash
philharmonica validate agent.json            # schema-check (no tokens spent)
philharmonica validate --resolve agent.json  # also import the tool references
philharmonica run agent.json "What time is it?"
...
A starter Philharmonica ADK multi-agent topology (triage → expert handoff).

```bash
philharmonica validate topology.json             # schema-check (no tokens spent)
philharmonica run topology.json "Explain BSP supersteps"
```
...
#   philharmonica run agent.json --env-file .env "hello"
# ANTHROPIC_API_KEY=
# OPENAI_API_KEY=
# GEMINI_API_KEY=
"""
...
def render_project(kind: str, name: str) -> dict[str, str]:
    ...
    """
    from philharmonica.adk.config.schema import dump_agent_config_schema, dump_topology_config_schema
