...
from philharmonica.adk.exceptions import MaxTurnsExceeded, ModelRefusalError, UsageLimitExceeded
from philharmonica.adk.llms.llm_usage import LLMUsage
from philharmonica.adk.run.cost import check_usage_limits
from philharmonica.adk.run.llm_calls import (
    call_llm,
    call_llm_streamed,
...
from philharmonica.adk.tracing import generation_span
from philharmonica.adk.types.items.items import ItemHelpers
from philharmonica.adk.verbose.hooks import (
...
from philharmonica.adk.verbose.run_bridge import active_verbose_agent, active_verbose_hooks
...
async def _activate_lazy_skills(
    ...
    """
    from philharmonica.adk.skills.skill_rendering import render_single_skill_instruction
...
async def resolve_system_prompt(
    ...
    the resolved prompt.
    """
    from philharmonica.adk.prompts.system_prompt import DynamicSystemPromptData, SystemPrompt
    ...
    if agent.skills and agent.skill_activation == SkillActivation.EAGER:
        from philharmonica.adk.skills.skill_rendering import render_skill_instructions
...
async def _apply_call_model_input_filter(
    *,
    ...
    from philharmonica.adk.run.config import CallModelData, ModelInputData
...
async def run_agent_block(
    ...
    while turn_offset + turn < max_turns:
        ...
        if jit_directives is not None and jit_directives.count > 0:
            from philharmonica.adk.context.directives import apply_directives
        ...
        from philharmonica.adk.run.next_step import (
            NextStepFinalOutput,
            NextStepHandoff,
        ...
        from philharmonica.adk.run.turn_resolution import (
...
async def run_agent_loop(
    ...
    """
    from philharmonica.adk.run.agent_middleware import compose_agent_middleware
    ...
    if config.context_management is not None:
        from philharmonica.adk.context import ContextManager
