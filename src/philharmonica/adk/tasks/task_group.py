...
A ``TaskGroup`` is the parallel counterpart to ``TaskPipeline``.
It bundles N tasks that run **concurrently** under
``asyncio.gather`` semantics, optionally bounded by a semaphore,
and aggregates per-task usage into a single
``TaskGroupResult.context``.

Use a ``TaskGroup`` when:
...
A ``TaskGroup`` is the wrong abstraction when tasks depend on each
other's outputs — use ``TaskPipeline`` for sequential chaining or
``Graph`` for arbitrary dependency DAGs.

Concurrency contract (developer-visible):

- ``on_task_start`` / ``on_task_end`` hooks fire **concurrently** when
  invoked from ``Runner.arun_task_group``. Hooks holding shared
  mutable state MUST lock — the framework does not synchronise.
- **Shared ``Session`` is not recommended.** When N parallel
  ...
  ``Session`` (or write to a downstream aggregator after each
  task completes).
...
- ``error_policy="halt_on_first"`` cancels still-running siblings via
  ...
  cancellation. Cancelled tasks appear in ``TaskGroupResult.task_outputs``
...
- ``TaskGroup.error_policy`` defaults to ``"collect_all"`` —
  every task runs to completion regardless of sibling failures.
- ``TaskGroup.max_concurrent`` defaults to ``None`` — concurrency
...
surface as ``TaskOutput`` slots with the ``error`` field set.

``"halt_on_first"`` — on the first task failure, the Runner cancels
all still-running siblings via ``asyncio.Task.cancel``. The
...
class TaskGroup[TContext]:
    """Parallel composition of ``Task`` instances.
    ...
    ``asyncio.gather``, optionally bounded by an
    ``asyncio.Semaphore`` when ``max_concurrent`` is set.

    Attributes:
        tasks: Ordered tuple of tasks to execute. Order determines
            positional indexing in
            ``TaskGroupResult.task_outputs``; execution order is
    ...
    Raises:
        ValueError: When ``tasks`` is empty, or when
            ``max_concurrent`` is set but non-positive.
    ...
    Example:
        ::
            ...
            from philharmonica.adk.tasks import TaskGroup
    ...
    def __post_init__(self) -> None:
        """Validate ``TaskGroup`` construction.
...
class TaskGroupResult[TContext]:
    """Result of a ``TaskGroup`` execution.

    Attributes:
        task_outputs: One ``TaskOutput`` per task in
            ``TaskGroup.tasks``, in the **same input order**.
            Execution order is asyncio-scheduler-dependent, but the
            result tuple preserves the position each task occupied in
            ``TaskGroup.tasks``. Failed tasks appear with
            ``error`` set; cancelled (by ``halt_on_first``) tasks
            appear with an explanatory ``error`` field.
        context: The ``RunContext`` for the whole group. Its
            ``usage`` field aggregates ``LLMUsage`` across every
            ...
            ``LLMUsage.__add__`` is commutative so the resulting
            totals are stable.
        metadata: Verbatim copy of ``TaskGroup.metadata``.
