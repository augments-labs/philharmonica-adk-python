...
A ``FlowCheckpoint`` records a Flow's between-step state so the run
...
- Any steps paused by a ``requires_approval`` gate
  (``deferred_steps``) so resume can replay them under one or
  more ``FlowApprovalDecision`` instances.

Persistence is **explicit** — there is no ``@persist`` decorator. The
developer calls ``FlowCheckpoint.capture`` (or constructs the
dataclass manually), serializes via ``to_json``, stores wherever
they choose, and resumes via
``Runner.arun_flow_from_checkpoint``. This explicit-only model
...
from philharmonica.adk.flows.deferred import FlowApprovalDecision, FlowDeferralKind, FlowDeferredStep
from philharmonica.adk.flows.triggers import FlowTriggerEvent, FlowTriggerKind
...
class FlowCheckpoint:
    ...
    Captures everything the ``FlowExecutor`` needs to resume the
    ...
    Attributes:
        flow_id: Stable identifier of the Flow instance — set by
            ``Flow.flow_id`` and preserved across the round-trip
    ...
    that is NOT in ``deferred_steps``.  On resume,
    ``Runner._seed_executor_from_checkpoint`` restores these into
    ``executor.pending_triggers`` so that ``FlowExecutor._build_step_context``
    ...
    """``step_name → FlowApprovalDecision`` recorded via ``approve`` / ``reject``.

    Mirrors ``RunState.deferred_tool_requests`` / the
    ``approved_tools`` + ``rejected_tools`` channels on RunState —
    decisions baked into the resume payload itself, no separate
    approvals kwarg on ``Runner.arun_flow_from_checkpoint``.

    The dict is mutable on a frozen dataclass: ``approve`` /
    ``reject`` mutate the inner mapping the way
    ``RunState.approve`` / ``RunState.reject`` mutate
    ...
    def requires_action(self) -> bool:
        ...
        Mirrors ``RunResult.requires_action`` at the checkpoint
        layer. Equivalent to
    ...
    def _require_known_step(self, deferred_step: FlowDeferredStep | str, *, method: str) -> str:
        ...
        Args:
            deferred_step: A ``FlowDeferredStep`` or a bare
                ``step_name`` string.
        ...
        Raises:
            TypeError: When ``deferred_step`` is neither a string nor a
                ``FlowDeferredStep`` — a bare ``AttributeError``
    ...
    def from_json(cls, payload: str) -> FlowCheckpoint:
        """Rehydrate a ``FlowCheckpoint`` from ``to_json`` output.
        ...
        ``ValueError``. Optional fields (``state_type_name``,
        ...
        Returns:
            A reconstructed ``FlowCheckpoint`` instance.
        ...
        *,
        ...
        Args:
            ...
            pending_step_triggers: ``{step_name: [FlowTriggerEvent, ...]}``
                ...
                ``FlowCheckpoint.pending_step_triggers``).
            decisions: Pre-recorded ``{step_name: FlowApprovalDecision}``
                mapping to seed ``FlowCheckpoint.decisions`` with;
                ``None`` starts empty.

        Returns:
            A new frozen ``FlowCheckpoint``.
        """
        return cls(
            flow_id=flow_id,
...
def _deferred_to_dict(deferred: FlowDeferredStep) -> dict[str, Any]:
    """Serialise one ``FlowDeferredStep`` to a dict for JSON.

    ``triggers`` is encoded as a list of dicts (one per
    ``FlowTriggerEvent``). Timestamps are ISO 8601. The
...
def _deferred_from_dict(data: dict[str, Any]) -> FlowDeferredStep:
    """Rehydrate a ``FlowDeferredStep`` from its dict form.

    Raises ``ValueError`` (not bare ``KeyError``) when a
...
def _trigger_from_dict(data: dict[str, Any]) -> FlowTriggerEvent:
    """Rehydrate one ``FlowTriggerEvent`` with loud missing-field validation."""
...
def _decision_to_dict(decision: FlowApprovalDecision) -> dict[str, Any]:
    """Serialise one ``FlowApprovalDecision`` to a dict for JSON.

    Includes ``expired`` so the typed ``FlowApprovalDecision.status``
...
def _decision_from_dict(data: dict[str, Any]) -> FlowApprovalDecision:
    """Rehydrate a ``FlowApprovalDecision`` from its dict form.

    Raises ``ValueError`` on missing required fields so a
...
def _require_field(data: dict[str, Any], field_name: str, *, context: str) -> None:
    """Raise ``ValueError`` when ``field_name`` is missing from ``data``.

    Provides callers with a flow-specific actionable message instead
    of a bare ``KeyError`` on the offending subscript.
    """
...
def _validate_deferral_kind(value: Any) -> FlowDeferralKind:
    """Narrow a deserialised string to the ``FlowDeferralKind`` literal.

    Raises ``ValueError`` on an unknown discriminator so a
...
def _validate_trigger_kind(value: Any) -> FlowTriggerKind:
    """Narrow a deserialised string to the ``FlowTriggerKind`` literal.
