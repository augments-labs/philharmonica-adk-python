"""FlowConfig — cost-conservative bounds for a ``Flow`` execution.
...
This config is a sibling to ``philharmonica.adk.run.config.RunConfig``. Both
travel through ``Runner.arun_flow``. ``RunConfig`` covers concerns
...
- ``"halt"`` — surfaces the exception immediately as
  ``philharmonica.adk.flows.exceptions.FlowStepError``. Cost-conservative
...
- ``"route_to_error_handler"`` — looks up a listener decorated as
  ...
  literal; see ``philharmonica.adk.flows.triggers.FLOW_ERROR_TRIGGER``)
...
class FlowConfig:
    """Cost-conservative bounds for a ``Flow`` execution.
    ...
    Attributes:
        max_steps: Hard cap on the total number of step invocations
            ...
            cap is hit, ``philharmonica.adk.flows.exceptions.FlowMaxStepsExceeded``
        ...
        max_listeners_per_step: Maximum number of downstream methods
            ...
            ``philharmonica.adk.flows.exceptions.FlowDefinitionError`` if
            exceeded.
        error_policy: How to handle exceptions raised by step bodies.
            Default ``"halt"``. See ``FlowErrorPolicy``.
    """
    ...
    def __post_init__(self) -> None:
        """Validate ``FlowConfig`` field values.
