...
from philharmonica.adk.mcp.notifications import make_message_handler
...
try:
    ...
    raise ImportError(
        "Please install the 'mcp' extra to use MCPServer. You can use the `mcp` optional group - pip install philharmonica-adk[mcp]"
    ) from ie
...
class MCPServer(abc.ABC):
    ...
    ``philharmonica.adk.types``; conversion happens in ``conversion.py``.
