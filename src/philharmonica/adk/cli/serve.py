...
For containers, bind all interfaces and read the platform port::
    ...
    philharmonica serve --agent app:agent --host 0.0.0.0 --port "$PORT"
...
from philharmonica.adk.cli.errors import framework_errors
from philharmonica.adk.cli.loading import load_env_file, primary_executable, resolve_target
from philharmonica.adk.cli.options import target_options
...
if TYPE_CHECKING:
    ...
    from philharmonica.adk.a2a.server import A2AServer
    from philharmonica.adk.agents.agent import Agent
    from philharmonica.adk.session.multi_sessions import MultiSessions
    from philharmonica.adk.types.session.store import SessionStore
...
SERVE_INSTALL_HINT = (
    "philharmonica serve requires the serve extra. Install with: pip install 'philharmonica-adk[serve]'"
)
A2A_INSTALL_HINT = "the A2A surface requires the a2a extra. Install with: pip install 'philharmonica-adk[a2a]'"
A2A_PG_HINT = "--task-dsn requires the a2a-postgres extra. Install with: pip install 'philharmonica-adk[a2a-postgres]'"
SESSION_PG_HINT = (
    "--session-dsn requires the session-postgres extra. Install with: pip install 'philharmonica-adk[session-postgres]'"
)
