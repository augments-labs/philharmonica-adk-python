"""DaytonaSandboxClient — hosted bridge backed by the Daytona sandbox provider.
...
_DEFAULT_BASE_URL = "https://app.daytona.io/api"


class DaytonaSandboxClientOptions(RemoteVMSandboxClientOptions):
    """Options for the Daytona hosted bridge."""

    api_key: str | None = None
    """Daytona auth token / API key. Required at create-time."""

    workspace_id: str | None = None
    """Daytona workspace identifier."""


class DaytonaSandboxClient(RemoteVMSandboxClient):
    """Hosted-bridge client backed by Daytona."""

    backend_id = "daytona"
    # Approximate compute rate (USD/min); override per current Daytona pricing:
    # https://www.daytona.io/pricing
    cost = SandboxCostDescriptor(usd_per_minute=0.08)
    ...
    def _auth_headers(self, options: DaytonaSandboxClientOptions) -> dict[str, str]:
        if options.api_key is None or len(options.api_key) == 0:
            raise SandboxStartFailed(
                backend_id="daytona",
                reason="DaytonaSandboxClientOptions.api_key is required at create-time.",
            )
        return {
    ...
    async def create(
        ...
        *,
        ...
        options: DaytonaSandboxClientOptions,  # type: ignore[override]
    ) -> BaseSandboxSession:
        ...
        try:
            body: dict[str, Any] = {"workspaceId": options.workspace_id} if options.workspace_id is not None else {}
            ...
            try:
                ...
                raise SandboxStartFailed(
                    backend_id="daytona",
                    reason=f"Daytona create failed: {exc}",
                ) from exc
            ...
            if not isinstance(sandbox_id, str) or len(sandbox_id) == 0:
                raise SandboxStartFailed(
                    backend_id="daytona",
                    reason=f"Daytona create did not return a sandbox ID; payload={payload!r}",
                )
    ...
    async def resume(self, state: SandboxSessionState) -> BaseSandboxSession:
        ...
        if not isinstance(sandbox_id, str) or len(sandbox_id) == 0:
            raise SandboxStartFailed(
                backend_id="daytona",
                reason="Daytona resume requires provider_payload['sandbox_id']",
            )
