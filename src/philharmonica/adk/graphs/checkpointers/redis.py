"""``RedisCheckpointer`` — fast, TTL-aware graph-run persistence via Redis.

Each checkpoint is a Redis hash under ``graph:ckpt:<thread_id>`` holding a
...
try:
    ...
    raise ImportError(
        "RedisCheckpointer requires redis>=5.0: pip install 'philharmonica-adk[checkpointer-redis]'"
    ) from exc
...
class RedisCheckpointer(Checkpointer):
    """Fast, TTL-aware graphs checkpointer backed by Redis hashes.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialise the checkpointer with a Redis connection.

        Args:
            client: A pre-configured ``redis.asyncio.Redis`` instance
                (e.g. for tests or a shared pool). Mutually exclusive
                with ``url``.
            url: A Redis URL (``redis://host:port/db``) used to build a
                client when ``client`` is not supplied. Mutually
                exclusive with ``client``.
            ttl_seconds: Optional per-key expiry in seconds. ``None``
                (the default) keeps checkpoints until explicitly deleted
                — the cost-conservative default; the developer opts in
                to eviction.

        Raises:
            ValueError: When both ``client`` and ``url`` are supplied,
                when neither is supplied, or when ``ttl_seconds`` is
                non-positive.
        """
        if client is not None and url is not None:
            raise ValueError("RedisCheckpointer accepts client= or url=, not both.")
        ...
        if client is not None:
            ...
            raise ValueError("RedisCheckpointer requires either client= or url=.")
        ...
        logger.debug("RedisCheckpointer initialised (ttl_ms=%d).", self._ttl_ms)
    ...
    async def close(self) -> None:
        ...
        if self._owns_client:
            ...
            logger.debug("RedisCheckpointer: client closed.")

    @override
    def register(self, registry: HookRegistry) -> None:
        """Subscribe to ``on_node_end`` / ``on_graph_end``."""
        registry.add(CheckpointerHooks(self))
        logger.debug("RedisCheckpointer registered on HookRegistry.")
    ...
    async def save(self, checkpoint: GraphCheckpoint) -> None:
        ...
        logger.debug(
            "RedisCheckpointer.save: thread_id=%s superstep=%s",
    ...
    async def load(self, thread_id: str, graph: Graph[Any]) -> GraphState[Any] | None:
        ...
        if len(raw) == 0:
            ...
            logger.debug("RedisCheckpointer.load: no checkpoint for thread_id=%s", thread_id)
            return None
        ...
        if envelope["graph_id"] != graph.id:
            raise ValueError(
                f"Checkpoint graph_id={envelope['graph_id']!r} does not match supplied graph.id={graph.id!r}. Refusing to load."
            )
        self._tokens[thread_id] = data["lock_token"]
        logger.debug("RedisCheckpointer.load: thread_id=%s graph_id=%s", thread_id, graph.id)
    ...
    async def delete(self, thread_id: str) -> None:
        ...
        self._tokens.pop(thread_id, None)
        logger.debug("RedisCheckpointer.delete: thread_id=%s", thread_id)
