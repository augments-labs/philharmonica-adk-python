...
| 4 | `requirements.txt` is the install seam, not a public index | The package is private, so the Dockerfile installs `philharmonica-adk[...]` from whatever the operator's `requirements.txt` points at (private index / vendored wheel / VCS). |
