...
Example::
    ...
    import dataclasses
    from philharmonica.adk.types.items import UserItem
...
class RunConfig:
    ...
    quotas, spans (``philharmonica.tenant.id``), the ``tenant`` metric dimension,
    ...
    (OTel emits them as ``philharmonica.metadata.<key>`` span attributes). Do
    ...
    **Safety-net semantics**: this is the "absolute safety net" for
    ...
    Example::
        ...
        from philharmonica.adk.agents import AgentGuardrails
    ...
    ``ModelRefusalError.__mro__`` → ``[ModelRefusalError, PhilharmonicaError, …]``
