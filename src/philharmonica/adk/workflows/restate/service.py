...
class PhilharmonicaRestateService:
    """Mixin providing common Restate service helpers for Philharmonica workflows.
    ...
    Example::
        ...
        helpers = PhilharmonicaRestateService()
    ...
    async def wait_for_human_reply(
        ...
        logger.info(
            "PhilharmonicaRestateService.wait_for_human_reply: awaiting promise %r",
            promise_name,
        )
        raw: dict[str, Any] = await ctx.promise(promise_name).value()
        logger.debug(
            "PhilharmonicaRestateService.wait_for_human_reply: promise %r resolved, node_id=%r",
            promise_name,
            raw.get("node_id"),
        )
