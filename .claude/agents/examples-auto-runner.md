...
Useful invocations (run from the repo root):

- `uv run python examples/run_examples.py --list [--filter X]` — classify only, no
  runs, zero API cost. ALWAYS start here to preview scope and cost.
- `uv run python examples/run_examples.py --auto-mode [--filter X]` — run.
- `uv run python examples/run_examples.py --auto-mode --rerun-file logs/run_examples/latest_failures.txt`
...
1. `uv run python examples/run_examples.py --list --filter <scope>` — review the
