    ...
    *,
    ...
    Args:
        ...
        convention: Span-attribute vocabulary the returned tracer emits.
            ...
            ``philharmonica.*``). Pass ``TracingConvention.OPENINFERENCE`` to
...
def setup_otel_from_env(
    ...
    * ``OTEL_SERVICE_NAME`` — value for the ``service.name`` resource
      attribute.  When absent the framework defaults to ``"philharmonica-adk"``.
    ...
    service_name = os.environ.get("OTEL_SERVICE_NAME", "philharmonica-adk")
    logger.debug(
