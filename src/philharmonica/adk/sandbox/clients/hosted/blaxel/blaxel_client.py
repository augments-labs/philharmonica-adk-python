"""BlaxelSandboxClient — hosted bridge backed by the Blaxel sandbox provider.
...
_DEFAULT_BASE_URL = "https://api.blaxel.ai"


class BlaxelSandboxClientOptions(RemoteVMSandboxClientOptions):
    """Options for the Blaxel hosted bridge."""

    api_key: str | None = None
    """Blaxel auth token / API key. Required at create-time."""

    workspace: str | None = None
    """Blaxel workspace identifier."""


class BlaxelSandboxClient(RemoteVMSandboxClient):
    """Hosted-bridge client backed by Blaxel."""

    backend_id = "blaxel"
    # Approximate compute rate (USD/min); override per current Blaxel pricing:
    # https://www.blaxel.ai/pricing
    cost = SandboxCostDescriptor(usd_per_minute=0.09)
    ...
    def _auth_headers(self, options: BlaxelSandboxClientOptions) -> dict[str, str]:
        if options.api_key is None or len(options.api_key) == 0:
            raise SandboxStartFailed(
                backend_id="blaxel",
                reason="BlaxelSandboxClientOptions.api_key is required at create-time.",
            )
        return {
    ...
    async def create(
        ...
        *,
        ...
        options: BlaxelSandboxClientOptions,  # type: ignore[override]
    ) -> BaseSandboxSession:
        ...
        try:
            body: dict[str, Any] = {"workspace": options.workspace} if options.workspace is not None else {}
            ...
            try:
                ...
                raise SandboxStartFailed(
                    backend_id="blaxel",
                    reason=f"Blaxel create failed: {exc}",
                ) from exc
            ...
            if not isinstance(sandbox_id, str) or len(sandbox_id) == 0:
                raise SandboxStartFailed(
                    backend_id="blaxel",
                    reason=f"Blaxel create did not return a sandbox ID; payload={payload!r}",
                )
    ...
    async def resume(self, state: SandboxSessionState) -> BaseSandboxSession:
        ...
        if not isinstance(sandbox_id, str) or len(sandbox_id) == 0:
            raise SandboxStartFailed(
                backend_id="blaxel",
                reason="Blaxel resume requires provider_payload['sandbox_id']",
            )
