...
from philharmonica.adk.hooks.hooks import RunHooks, compose_run_hooks
from philharmonica.adk.run.config import DEFAULT_MAX_TURNS, DEFAULT_RUN_CONFIG, RunConfig
from philharmonica.adk.run.context import RunContext, TContext
...
from philharmonica.adk.run.loop import run_agent_loop, run_agent_loop_streamed
...
from philharmonica.adk.run.stream import CancelMode, HookEventKind, HookLifecycleEvent, RunResultStreaming
from philharmonica.adk.tracing import agent_span
from philharmonica.adk.tracing.spans import swarm_span
...
if TYPE_CHECKING:
    ...
    from philharmonica.adk.flows.flow import Flow
    from philharmonica.adk.flows.result import FlowRunResult, FlowRunResultStreaming
    from philharmonica.adk.flows.worker_backend import FlowWorkerBackend
    from philharmonica.adk.graphs.checkpointer import Checkpointer
    from philharmonica.adk.graphs.graph import Graph
    from philharmonica.adk.graphs.hooks import GraphHooks, HookProvider
    ...
    from philharmonica.adk.run.profile import RunnerProfile
    from philharmonica.adk.run.types import UserPrompt
    from philharmonica.adk.swarms.checkpointer import SwarmCheckpointer
    from philharmonica.adk.swarms.interrupt import SwarmResume
    from philharmonica.adk.swarms.result import SwarmRunResult, SwarmRunResultStreaming
    ...
    from philharmonica.adk.tasks.task_group import TaskGroup, TaskGroupResult
    from philharmonica.adk.tasks.task_output import TaskOutput
    from philharmonica.adk.tasks.task_pipeline import TaskPipeline, TaskPipelineResult
    from philharmonica.adk.tasks.task_pipeline_state import TaskPipelineState
    from philharmonica.adk.tracing.spans import AgentSpanData, Span
    ...
    from philharmonica.adk.types.input.llm_input_text import LLMInputText
...
def wrap_hooks_with_verbose(
    ...
    if config.verbose is not None and config.verbose.enabled:
        from philharmonica.adk.verbose.hooks import VerboseHooks, find_verbose_hooks
...
def _activate_mcp_run_hooks_bridge(hooks: RunHooks[Any], ctx: Any) -> None:
    ...
    """
    try:
        from philharmonica.adk.mcp.run_hooks_bridge import active_run_context, active_run_hooks
    except ImportError:
        return
...
async def _dispose_agent_toolsets(agent: Agent) -> None:
    ...
    """
    from philharmonica.adk.tools.toolsets import Toolset
...
class Runner:
    ...
    async def arun_swarm(
        ...
        *,
        ...
        """
        # Deferred import: swarm_loop pulls in philharmonica.adk.swarms.* which
        # transitively imports from philharmonica.adk.handoffs.handoff. Hoisting
        ...
        from philharmonica.adk.session.session_event import (
            SessionEvent,
            create_session_event,
        )
        ...
        *,
        ...
        if regenerated and initial_state is not None:
            logger.warning(
                ...
                "work — the new root span starts a fresh philharmonica.swarm.id."
            )
        ...
        *,
        ...
        if loaded_state.swarm_id is not None:
            ...
            logger.warning(
                ...
                "starts a fresh philharmonica.swarm.id."
            )
    ...
    async def arun_task_pipeline(
        ...
        *,
        ...
        Returns:
            ...
            output.
        """
        from philharmonica.adk.tasks.task_pipeline import TaskPipelineResult
        ...
        *,
        ...
        Raises:
            FlowCheckpointNotFoundError: When ``checkpoint_id`` is not
                found in ``backend``.
        """
        from philharmonica.adk.flows.exceptions import FlowCheckpointNotFoundError
    ...
    async def _run_flow_batch_concurrent(
        ...
        *,
        ...
        # `_run_one_in_batch` absorbs every per-item PhilharmonicaError into a
    ...
    async def _run_one_in_batch(
        ...
        *,
        ...
        from philharmonica.adk.flows.result import FlowRunResult
...
def _compose_task_prompt(
    ...
    from philharmonica.adk.tasks.task_input_data import TaskInputData
...
def _encode_flow_state(flow: Any) -> str:
    ...
    stay in one place.
    """
    from philharmonica.adk.flows.executor import encode_state
