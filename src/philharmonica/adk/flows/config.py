...
This config is a sibling to :class:`philharmonica.adk.run.config.RunConfig`. Both
...
- ``"halt"`` — surfaces the exception immediately as
  :class:`philharmonica.adk.flows.exceptions.FlowStepError`. Cost-conservative
...
- ``"route_to_error_handler"`` — looks up a listener decorated as
  ...
  literal; see :data:`philharmonica.adk.flows.triggers.FLOW_ERROR_TRIGGER`)
...
class FlowConfig:
    ...
    Attributes:
        max_steps: Hard cap on the total number of step invocations
            ...
            cap is hit, :class:`philharmonica.adk.flows.exceptions.FlowMaxStepsExceeded`
        ...
        max_total_tokens: Optional cumulative cap on tokens consumed by
            ...
            to :attr:`philharmonica.adk.run.config.RunConfig.max_total_tokens`.
        max_listeners_per_step: Maximum number of downstream methods
            ...
            :class:`philharmonica.adk.flows.exceptions.FlowDefinitionError` if
            exceeded.
