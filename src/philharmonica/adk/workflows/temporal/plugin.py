...
from philharmonica.adk.workflows.temporal.activity import register_model as activity_register_model
from philharmonica.adk.workflows.temporal.determinism import (
    DEFAULT_PASSTHROUGH_MODULES,
    build_sandbox_restrictions,
)
from philharmonica.adk.workflows.temporal.serialization import build_philharmonica_data_converter
...
class PhilharmonicaTemporalPlugin(ClientPlugin, WorkerPlugin):
    """Client + worker plugin wiring Philharmonica Temporal workflows.

    On the client side, installs the Philharmonica data converter. On the
    worker side, installs a sandboxed workflow runner whose passthrough
    modules cover the Philharmonica ADK and its dependencies, and syncs the
    ...
    def __post_init__(self) -> None:
        ...
        logger.info(
            "PhilharmonicaTemporalPlugin initialized with %d passthrough modules",
    ...
    def register_model(self, name: str, llm: LLM) -> None:
        ...
        logger.info("PhilharmonicaTemporalPlugin registered model %r", name)

    # -- temporalio.client.Plugin surface ---------------------------------

    @override
    def configure_client(self, config: ClientConfig) -> ClientConfig:
        """Install the Philharmonica data converter on the client.

        The converter is a client-level setting in temporalio; workers
        and workflow handles created from the client inherit it.
        """
        config["data_converter"] = build_philharmonica_data_converter()
        logger.debug("PhilharmonicaTemporalPlugin: data converter installed on client config")
        return config
    ...
    def configure_worker(self, config: WorkerConfig) -> WorkerConfig:
        ...
        logger.debug("PhilharmonicaTemporalPlugin: sandboxed workflow runner installed on worker config")
        return config
