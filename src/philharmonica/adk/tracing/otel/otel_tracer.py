...
from philharmonica.adk.tracing.openinference import conventions as oi
...
def _flatten_attributes(exported: dict[str, Any], *, prefix: str = "") -> dict[str, Any]:
    ...
    Separator semantics::
        ...
        _flatten_attributes({"a": 1}, prefix="philharmonica")  → {"philharmonica.a": 1}
...
def _agent_attrs(data: AgentSpanData) -> dict[str, Any]:
    attrs: dict[str, Any] = {
        "philharmonica.agent.name": data.name,
    }
    if data.handoffs is not None and len(data.handoffs) > 0:
        attrs["philharmonica.agent.handoffs"] = list(data.handoffs)
    if data.tools is not None and len(data.tools) > 0:
        attrs["philharmonica.agent.tools"] = list(data.tools)
    if data.output_type is not None:
        attrs["philharmonica.agent.output_type"] = data.output_type
    for k, v in data.metadata.items():
        attrs.update(_flatten_attributes({k: v}, prefix="philharmonica.metadata"))
    if data.tenant_id is not None:
        attrs["philharmonica.tenant.id"] = data.tenant_id
    return attrs
...
def _function_attrs(
    ...
    *,
    ...
    attrs: dict[str, Any] = {
        "philharmonica.tool.name": data.name,
    }
    if data.input is not None:
        attrs["philharmonica.tool.input"] = data.input if record_full else _redact_and_truncate(data.input, max_chars)
    if data.output is not None:
        raw_output = str(data.output)
        attrs["philharmonica.tool.output"] = raw_output if record_full else _redact_and_truncate(raw_output, max_chars)
    if data.mcp_data is not None:
        attrs.update(_flatten_attributes(data.mcp_data, prefix="philharmonica.mcp"))
    if data.a2a_data is not None:
        ...
        attrs.update(_flatten_attributes(a2a_safe, prefix="philharmonica.a2a"))
    return attrs


def _generation_attrs(data: GenerationSpanData) -> dict[str, Any]:
    attrs: dict[str, Any] = {"gen_ai.system": "philharmonica"}
...
def _handoff_attrs(data: HandoffSpanData) -> dict[str, Any]:
    ...
    if data.from_agent is not None:
        attrs["philharmonica.handoff.from"] = data.from_agent
    if data.to_agent is not None:
        attrs["philharmonica.handoff.to"] = data.to_agent
    return attrs


def _guardrail_attrs(data: GuardrailSpanData) -> dict[str, Any]:
    return {
        "philharmonica.guardrail.name": data.name,
        "philharmonica.guardrail.triggered": data.triggered,
    }


def _custom_attrs(data: CustomSpanData) -> dict[str, Any]:
    attrs: dict[str, Any] = {"philharmonica.span.name": data.name}
    attrs.update(_flatten_attributes(data.data, prefix="philharmonica.custom"))
    return attrs
...
def _graph_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    if graph_id is not None:
        attrs["philharmonica.graph.id"] = graph_id
    entry = payload.get("entry")
    if entry is not None:
        attrs["philharmonica.graph.entry"] = entry
    status = payload.get("status")
    if status is not None:
        attrs["philharmonica.graph.status"] = status
    supersteps_total = payload.get("supersteps_total")
    if supersteps_total is not None:
        attrs["philharmonica.graph.supersteps_total"] = supersteps_total
    return attrs
...
def _graph_superstep_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    ``philharmonica.graph.superstep.*``. The parent ``philharmonica.graph.id`` is
    ...
    if index is not None:
        attrs["philharmonica.graph.superstep.index"] = index
    ready_nodes = payload.get("ready_nodes")
    if ready_nodes is not None and len(ready_nodes) > 0:
        attrs["philharmonica.graph.superstep.ready_nodes"] = list(ready_nodes)
    fired_nodes = payload.get("fired_nodes")
    if fired_nodes is not None and len(fired_nodes) > 0:
        attrs["philharmonica.graph.superstep.fired_nodes"] = list(fired_nodes)
    return attrs
...
def _graph_node_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    ``philharmonica.graph.node.*``. ``resume_attempt`` only appears when the
    ...
    if node_name is not None:
        attrs["philharmonica.graph.node.name"] = node_name
    attempts = payload.get("attempts")
    if attempts is not None:
        attrs["philharmonica.graph.node.attempts"] = attempts
    status = payload.get("status")
    if status is not None:
        attrs["philharmonica.graph.node.status"] = status
    duration_ms = payload.get("duration_ms")
    if duration_ms is not None:
        attrs["philharmonica.graph.node.duration_ms"] = duration_ms
    resume_attempt = payload.get("resume_attempt")
    if resume_attempt is not None:
        attrs["philharmonica.graph.node.resume_attempt"] = resume_attempt
    return attrs
...
def _swarm_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    if swarm_id is not None:
        attrs["philharmonica.swarm.id"] = swarm_id
    entry = payload.get("entry")
    if entry is not None:
        attrs["philharmonica.swarm.entry"] = entry
    status = payload.get("status")
    if status is not None:
        attrs["philharmonica.swarm.status"] = status
    turns_total = payload.get("turns_total")
    if turns_total is not None:
        attrs["philharmonica.swarm.turns_total"] = turns_total
    return attrs
...
def _swarm_turn_attrs(data: CustomSpanData) -> dict[str, Any]:
    ...
    ``philharmonica.swarm.turn.*``. The parent ``philharmonica.swarm.id`` is repeated
    ...
    index = payload.get("index")
    if index is not None:
        attrs["philharmonica.swarm.turn.index"] = index
    member = payload.get("member")
    if member is not None:
        attrs["philharmonica.swarm.turn.member"] = member
    status = payload.get("status")
    if status is not None:
        attrs["philharmonica.swarm.turn.status"] = status
    duration_ms = payload.get("duration_ms")
    if duration_ms is not None:
        attrs["philharmonica.swarm.turn.duration_ms"] = duration_ms
    resume_attempt = payload.get("resume_attempt")
    if resume_attempt is not None:
        attrs["philharmonica.swarm.turn.resume_attempt"] = resume_attempt
    return attrs
        ...
        *,
        ...
        Args:
            ...
            convention: Span-attribute vocabulary to emit. Defaults to
                ...
                ``philharmonica.*``). Pass ``TracingConvention.OPENINFERENCE``
