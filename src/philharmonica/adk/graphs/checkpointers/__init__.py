"""Concrete ``Checkpointer`` implementations.

``InMemoryCheckpointer`` — a dict-backed, process-local store for
tests, notebooks, and single-process demos.

``SQLiteCheckpointer`` — a durable, single-file store (one row per
``thread_id``, latest wins) for multi-process or crash-recoverable runs.

``TieredCheckpointer`` — a hot/cold composite that writes to a fast
...
``philharmonica.adk.graphs.checkpointers.postgres`` (not re-exported here to keep
the optional dependency out of the default import path).

``RedisCheckpointer`` — a fast,
...
``philharmonica.adk.graphs.checkpointers.redis`` (not re-exported here to keep the
optional dependency out of the default import path).

``S3Checkpointer`` — an archival,
...
``philharmonica.adk.graphs.checkpointers.s3`` (not re-exported here to keep the
optional dependency out of the default import path).
"""
