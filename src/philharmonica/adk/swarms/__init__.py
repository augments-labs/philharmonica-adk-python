...
- **Swarm = config, Runner = execution.** No ``Swarm.run()``. The
  driver lives in ``philharmonica.adk.run.swarm_loop``.
...
``philharmonica.adk.run.swarm_loop`` (non-streaming) and
``philharmonica.adk.run.swarm_loop_streamed`` (event-emitting variant);
runner / builder integration lives in ``philharmonica.adk.run.runner``.
"""
...
from philharmonica.adk.swarms.config import SharedContextConfig, SwarmConfig
...
from philharmonica.adk.swarms.interrupt import SwarmResume, request_human_input_in_swarm
...
from philharmonica.adk.swarms.state import (
    SwarmState,
    SwarmStateDict,
)
from philharmonica.adk.swarms.stop_reason import StopReason
from philharmonica.adk.swarms.swarm import DEFAULT_MAX_TURNS, DEFAULT_TERMINATION, Swarm
...
from philharmonica.adk.swarms.yield_signal import (
