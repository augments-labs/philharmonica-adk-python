...
The `philharmonica` console script (`[project.scripts]` → `philharmonica.adk.cli:main`,
also `python -m philharmonica.adk.cli`). Click-based; one module per command.
...
| `__main__.py` | `python -m philharmonica.adk.cli` entry |
...
| `run.py` | `philharmonica run` + shared `build_run_config`, `open_session`, `echo_stream`, `target_app_name` |
| `chat.py` | `philharmonica chat` — CLI-owned REPL (NOT a `run_demo_loop` wrapper: the demo loop has no `session=`) |
| `validate.py` | `philharmonica validate` (`--resolve` assembles refs) |
| `schema.py` | `philharmonica schema agent\|node\|topology` |
| `sessions.py` | `philharmonica sessions list\|show\|delete` |
| `new.py` + `templates.py` | `philharmonica new` scaffolding (string.Template, `$name`; schema files written beside the config) |
| `serve.py` | `philharmonica serve` — REST + health by default via `serving.build_app`, A2A added with `--card`, run under uvicorn; behind the `serve` extra |

## Key decisions

| Decision | Rationale |
|---|---|
| click is a core dep | Zero transitive deps on POSIX; the entry point must work on a base install (first-touch UX: `pip install philharmonica-adk && philharmonica new`). The upstream mistake to avoid was making the *web/server stack* core, not click. |
...
| Heavy framework imports live inside command bodies | Fast `--help`; the parent `philharmonica.adk` import is already paid, but command modules must not add more at import time. |
...
| uvicorn imported only inside `serve` | The framework deliberately doesn't own the ASGI runtime; the `serve` extra (`philharmonica-adk[server,a2a]` + uvicorn) gates it with a guiding message. `serve` routes through `serving.build_app`, never importing the app stack at module load. |
