...
inline for a single example. Run from the repo root; `uv run` resolves the
locked environment without activating a shell.
...
```bash
uv run python examples/run_examples.py --list                 # whole suite
uv run python examples/run_examples.py --list --filter flows  # one topic
```
...
```bash
uv run python examples/run_examples.py --auto-mode --filter <topic>   # scoped
...
```bash
uv run python examples/run_examples.py --auto-mode --filter <path/to/example.py>
```

Re-run only the prior failures with:

```bash
uv run python examples/run_examples.py --auto-mode \
