...
``apply_*_to_k8s_pod`` helpers in ``philharmonica.adk.sandbox.policy``.
"""
...
def _require_k8s() -> None:
    try:
        ...
        raise UnsupportedSandboxClientError(
            "K8sPodSandboxClient requires the optional 'kubernetes' package: pip install 'philharmonica-adk[sandbox-k8s]'",
        ) from exc
...
def _build_pod_manifest(options: K8sSandboxClientOptions, pod_name: str) -> dict[str, Any]:
    ...
    labels: dict[str, str] = {
        "app.kubernetes.io/managed-by": "philharmonica-adk",
        "philharmonica.adk.io/role": "sandbox",
    }
    ...
    labels["philharmonica.adk.io/pss"] = options.pod_security_standard
    ...
    labels["philharmonica.sandbox/pod"] = pod_name
...
class K8sPodSandboxClient(BaseSandboxClient[K8sSandboxClientOptions]):
    ...
    async def create(
        ...
        *,
        ...
        pod_name = f"philharmonica-sandbox-{uuid.uuid4().hex[:12]}"
        pod_body = _build_pod_manifest(options, pod_name=pod_name)
        if manifest is not None:
            from philharmonica.adk.sandbox.policy import apply_mounts_to_k8s_pod
