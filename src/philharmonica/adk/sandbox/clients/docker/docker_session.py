...
from philharmonica.adk.sandbox.clients.session import (
    BaseSandboxSession,
    FileEntry,
...
class DockerSandboxSession(BaseSandboxSession):
    ...
    async def apply_patch(
        ...
        *,
        ...
        await self.write(".philharmonica_patch.diff", BytesIO(patch.encode("utf-8")))
        try:
            result = await self.run(
                "patch",
                "-p1",
                "-i",
                ".philharmonica_patch.diff",
                shell=False,
            )
        finally:
            with contextlib.suppress(Exception):
                await self.rm(".philharmonica_patch.diff")
        if result.exit_code != 0:
