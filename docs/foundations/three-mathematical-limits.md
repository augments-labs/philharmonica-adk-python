...
- Where formal verification is excluded by Rice, empirical evaluation is
  the only credible substitute — which is why benchmarking gets a project
  of its own rather than a corner of this one.
- The ADK's job is to make that measurement possible. Structured run
  results, tracing spans, and per-step verbose rendering are the
  observation surface through which correctness is *measured*, since it
  cannot be *proven*.
