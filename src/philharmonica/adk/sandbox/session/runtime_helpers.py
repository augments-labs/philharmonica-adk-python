...
_HELPER_INSTALL_ROOT: Final[PurePosixPath] = PurePosixPath("/tmp/philharmonica-adk/bin")
...
class RuntimeHelperScript:
    ...
    def install_command(self) -> tuple[str, ...]:
        ...
        heredoc = f"PHILHARMONICA_AGENTS_HELPER_{self.install_path.name.upper().replace('-', '_')}"
        return (
            "sh",
            "-c",
            f"""
