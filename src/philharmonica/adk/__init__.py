"""Philharmonica Agents ADK - A lightweight, provider-agnostic Python framework for building multi-agent workflows.
...
Example (streaming):
    from philharmonica.adk import Agent, Runner, RunItemType
...
Example (with LLMConfig):
    from philharmonica.adk import Agent, Runner, LLMConfig
...
_logger = _logging.getLogger("philharmonica.adk")
...
def setup_logging() -> None:
    ...
    Users can reconfigure after import:
        logging.getLogger("philharmonica.adk").setLevel(logging.WARNING)
    """
    # src/philharmonica/adk/__init__.py → 4 levels up to project root
        ...
        try:
            ...
            if "filename" in file_cfg:
                ...
                existing_logs = sorted(
                    log_dir.glob("philharmonica_adk.*.log"),
                ...
                if reusable is not None:
                    ...
                    file_cfg["filename"] = str(log_dir / f"philharmonica_adk.{timestamp}.log")
...
from philharmonica.adk.agents import (
    Agent,
...
from philharmonica.adk.session import Session, SessionSettings, SQLiteMultiSessions
...
from philharmonica.adk.tools import DocumentSearchTool, FunctionTool, MemoryTool
