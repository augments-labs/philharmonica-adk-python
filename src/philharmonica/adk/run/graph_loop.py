...
from philharmonica.adk.graphs.hooks import HookProvider, HookRegistry
...
from philharmonica.adk.run.node_reliability import resolve_node_reliability, run_node_with_reliability
from philharmonica.adk.run.state import RunState
from philharmonica.adk.run.stream import CancelMode
from philharmonica.adk.tracing.spans import Span, graph_node_span, graph_span, graph_superstep_span
...
async def _dispatch_node(
    *,
    ...
    # philharmonica.graph.node.status. InterruptException is a cooperative
    ...
    # philharmonica.graph.node.resume_attempt; original firings leave the
...
async def run_graph_loop(
    *,
    ...
    with graph_span(graph_id=graph.id, entry=graph.entry) as graph_tracing_span:
        ...
        # the OTel attribute mapping surfaces philharmonica.graph.status and
        # philharmonica.graph.supersteps_total. Same cast pattern as the
