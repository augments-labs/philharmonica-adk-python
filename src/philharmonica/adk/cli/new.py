"""``philharmonica new`` — scaffold a runnable agent project.
...
example, and a README — a project ``philharmonica validate --resolve`` passes
with no edits.
"""
...
from philharmonica.adk.cli.templates import render_project
...
def new(name: str, kind: str, parent_dir: Path) -> None:
    ...
    click.echo(f"  philharmonica validate {target_dir / config_name}")
    click.echo(f'  philharmonica run {target_dir / config_name} "hello"')
