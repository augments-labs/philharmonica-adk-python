...
from philharmonica.adk.exceptions import NoRoutingCandidateError, PhilharmonicaError, UsageLimitExceeded
...
from philharmonica.adk.run.cost import check_request_limit_before_call, check_usage_limits
from philharmonica.adk.types.input import LLMInputContentItem
from philharmonica.adk.verbose.run_bridge import fire_stream_chunk
...
if TYPE_CHECKING:
    ...
    from philharmonica.adk.llms import LLM
    ...
    from philharmonica.adk.run.tools_executor import FunctionToolFailureCounts
    ...
    from philharmonica.adk.tools.local.apply_patch_tool import ApplyPatchTool
    from philharmonica.adk.tools.local.shell_tool import ShellTool
    from philharmonica.adk.types.responses.llm_response import LLMResponse
    from philharmonica.adk.types.tools import ToolChoice
...
def _current_period_key(period: BudgetPeriod) -> str:
    ...
    from philharmonica.adk.budgets import period_key
...
def resolve_model_name(agent: Agent, config: RunConfig) -> str:
    ...
    """
    from philharmonica.adk.llms import LLM as _LLM
...
def resolve_output_schema(agent: Agent) -> AgentOutputSchemaBase | None:
    ...
    from philharmonica.adk.schemas import AgentOutputSchemaBase as _Base
...
async def enforce_tenant_budget(
    ...
    **closed** by default (``RunConfig.ledger_fail_open=False``): the period is
    ...
    from philharmonica.adk.verbose.hooks import emit_budget_exceeded
...
async def build_tools(
    ...
    from philharmonica.adk.context.context_config import CacheStrategy
    from philharmonica.adk.tools.builtin.builtin_tool import BuiltinTool, ExecutableBuiltinTool
    from philharmonica.adk.tools.function_tool import FunctionTool
    ...
    if len(conflicts) > 0:
        from philharmonica.adk.exceptions import ToolsetNameConflictError
    ...
    if agent.tools:
        ...
        for candidate in all_tools:
            ...
            if isinstance(tool, FunctionTool):
                ...
                from philharmonica.adk.tools.local.apply_patch_tool import ApplyPatchTool as _ApplyPatchTool
                from philharmonica.adk.tools.local.shell_tool import ShellTool as _ShellTool
    ...
    if agent.skills:
        from philharmonica.adk.skills.skill_rendering import check_skill_enabled
...
async def call_llm(
    ...
    from philharmonica.adk.llms.llm_middleware import compose_llm_middleware
...
def _is_routing_retryable(exc: Exception) -> bool:
    ...
    Framework errors (``PhilharmonicaError`` — budget exceeded, guardrail trips,
    ...
    """
    return not isinstance(exc, PhilharmonicaError)
...
async def _record_rejected_routed_response(
    *,
    ...
    from philharmonica.adk.verbose.hooks import emit_budget_exceeded, emit_usage_recorded
...
async def call_llm_streamed(
    ...
    from philharmonica.adk.run.stream import RawResponseStreamEvent
...
def find_agent_by_name(root_agent: Agent, name: str) -> Agent | None:
    ...
    while queue:
        ...
        from philharmonica.adk.handoffs.handoff import Handoff as _Handoff
