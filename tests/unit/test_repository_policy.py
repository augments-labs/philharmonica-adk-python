...
def test_release_smoke_test_imports_resolve() -> None:
    """Every import in the release smoke test must actually exist.

    The smoke test's heredoc only runs during a real release, so a symbol
    that is not exported at the level it is imported from would not surface
    until the publish job — after the tag already exists.
    """
    workflow = _read(".github/workflows/release-tag.yml")
    ...
    for statement in statements:
        # The executed source is this repo's own workflow file, not input.
        exec(statement, {})
