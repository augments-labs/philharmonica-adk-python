...
``register_llm_provider`` exposes the registry for extension. The built-in
factories are registered at import time.
...
def register_llm_provider(name: str, factory: ProviderFactory) -> None:
    ...
    Registering a name the schema's ``LLMProviderConfig`` union does not
    accept leaves an unreachable entry — Pydantic rejects an unknown ``provider`` block before
